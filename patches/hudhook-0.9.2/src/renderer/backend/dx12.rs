...
impl D3D12RenderEngine {
    ...
    pub fn new(
        ...
        let (rtv_heap, texture_heap) =
            unsafe { create_heaps(&device, &command_queue, node_mask) }?;
...
unsafe fn create_heaps(
    ...
    let texture_heap = TextureHeap::new(device, command_queue, srv_heap, node_mask)?;
...
impl TextureResourceState {
    const fn d3d12(self) -> D3D12_RESOURCE_STATES {
        match self {
            Self::CopyDestination => D3D12_RESOURCE_STATE_COPY_DEST,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct TextureUploadBarrierPlan {
    before_copy: Option<(TextureResourceState, TextureResourceState)>,
    after_copy: (TextureResourceState, TextureResourceState),
}

impl TextureUploadBarrierPlan {
    const fn for_stable_state(stable_state: TextureResourceState) -> Self {
        let before_copy = match stable_state {
            TextureResourceState::CopyDestination => None,
            TextureResourceState::PixelShaderResource => Some((
                TextureResourceState::PixelShaderResource,
                TextureResourceState::CopyDestination,
            )),
        };
    ...
    const fn stable_state_after_upload(self) -> TextureResourceState {
        self.after_copy.1
    }
}
...
struct Texture {
    ...
    width: u32,
    height: u32,
    stable_state: TextureResourceState,
}
...
impl TextureHeap {
    fn new(
        ...
        // Use the renderer's direct queue so D3D12 queue ordering keeps replacement uploads
        // behind every previously submitted frame that may still sample the same texture. The
        // upload fence below then completes before the current frame records its draw commands.
        let command_queue = command_queue.clone();
    ...
    unsafe fn create_texture(&mut self, width: u32, height: u32) -> Result<TextureId> {
        ...
        self.textures.push(Texture {
            ...
            width,
            height,
            stable_state: TextureResourceState::CopyDestination,
        });
    ...
    unsafe fn upload_texture(
        ...
        let barrier_plan = TextureUploadBarrierPlan::for_stable_state(texture.stable_state);
        let before_copy_barrier = barrier_plan.before_copy.map(|(before, after)| {
            util::create_barrier(&texture.resource, before.d3d12(), after.d3d12())
        });
        if let Some(barrier) = before_copy_barrier.as_ref() {
        ...
        let after_copy_barrier = util::create_barrier(
            &texture.resource,
            barrier_plan.after_copy.0.d3d12(),
            barrier_plan.after_copy.1.d3d12(),
        );

        self.command_list.ResourceBarrier(std::slice::from_ref(&after_copy_barrier));
        let upload_result = (|| -> Result<()> {
            self.command_list.Close()?;
            self.command_queue.ExecuteCommandLists(&[Some(self.command_list.cast()?)]);

            // ExecuteCommandLists has no return value. Once submitted, the final
            // barrier makes PIXEL_SHADER_RESOURCE the stable state even if fence
            // signaling or waiting reports a later device error.
            self.textures[texture_index].stable_state = barrier_plan.stable_state_after_upload();
        ...
        if let Some(barrier) = before_copy_barrier {
            util::drop_barrier(barrier);
        }
        util::drop_barrier(after_copy_barrier);
...
mod tests {
    ...
    #[test]
    fn texture_upload_barriers_cover_initial_and_repeated_uploads() {
        let initial =
            TextureUploadBarrierPlan::for_stable_state(TextureResourceState::CopyDestination);
        assert_eq!(initial.before_copy, None);
        assert_eq!(
            initial.after_copy,
            (TextureResourceState::CopyDestination, TextureResourceState::PixelShaderResource,)
        );

        let repeated =
            TextureUploadBarrierPlan::for_stable_state(initial.stable_state_after_upload());
        assert_eq!(
            repeated.before_copy,
            Some((
                TextureResourceState::PixelShaderResource,
                TextureResourceState::CopyDestination,
            ))
        );
        assert_eq!(repeated.after_copy, initial.after_copy);
        assert_eq!(repeated.stable_state_after_upload(), TextureResourceState::PixelShaderResource);
    }
