...
bitflags! {
    ...
    pub struct MessageFilter: u32 {
        ...
        /// Blocks keyboard input except Escape, allowing the host game to open or close its own
        /// menu while an overlay search field owns the remaining keyboard input.
        const InputKeyboardExceptEscape = 1u32 << 3;
...
impl MessageFilter {
    ...
    pub(crate) fn is_blocking(&self, message_id: u32, wparam: WPARAM) -> bool {
        if match message_id {
        ...
        match message_id {
            WM_KEYFIRST..=WM_KEYLAST => {
                self.contains(Self::InputKeyboard)
                    || (self.contains(Self::InputKeyboardExceptEscape)
...
mod tests {
    ...
    #[test]
    fn keyboard_except_escape_blocks_search_input_but_preserves_game_escape() {
        let filter = MessageFilter::InputKeyboardExceptEscape;

        assert!(!filter.is_blocking(WM_KEYDOWN, WPARAM(usize::from(VK_ESCAPE.0))));
        assert!(!filter.is_blocking(WM_KEYUP, WPARAM(usize::from(VK_ESCAPE.0))));
        assert!(filter.is_blocking(WM_KEYDOWN, WPARAM(usize::from(b'F'))));
        assert!(filter.is_blocking(WM_CHAR, WPARAM(usize::from(b'f'))));
    }
}
